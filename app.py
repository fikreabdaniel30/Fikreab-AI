...
# --- 2. ADVANCED CUSTOM CSS (ORIGINAL 10/10 DESIGN) ---
st.markdown("""
    <style>
...
# --- 4. THE "UNIVERSAL FIX" (CORRECTED INDENTATION) ---
model = None
if "GEMINI_API_KEY" in st.secrets:
    try:
        ...
        # DYNAMIC FIX: Ask API for compatible models
        available_models = []
        ...
        # THIS IS THE PART THAT WAS BROKEN IN YOUR GITHUB SHOT
        if available_models:
            # Pick a Flash model if possible, otherwise pick the first one
            ...
            st.sidebar.error("No compatible models found for this key.")
        ...
        if generate_btn:
            if model:
                with st.spinner(f"⏳ Generating {ai_mode}..."):
                    prompts = {
                        "📝 Comprehensive Notes": "Detailed notes with headers.",
                        "🗂️ Flashcard Set": "Term: Definition pairs.",
                        "📉 Key Points Only": "Bullet points only.",
                        "🎯 Exam Predictions": "Top topics and questions."
                    }
                    try:
